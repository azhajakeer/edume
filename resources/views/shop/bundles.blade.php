...
<x-app-layout>
    ...
    <div class="bg-white py-12 ">
        <div class="container mx-auto px-4">
            <!-- Title Section -->
            <h1 class="text-center text-4xl font-bold text-gray-800 dark:text-gray-900 mb-10">
                    ...
                    @foreach($bundles as $bundle)
                        <div class="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-xl shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105">
                            ...
                            <div class="p-5 text-center">
                                <h5 class="text-lg font-semibold text-white dark:text-gray-100 truncate">
                                    {{ $bundle->bundle_name }}
                                </h5>
                                <p class="text-gray-600 dark:text-grey-200 text-m mt-2">
                                    {{ Str::limit($bundle->description, 80) }}
                                </p>
                                <p class="text-indigo-600 dark:text-white font-bold text-lg mt-4">
                                    ${{ number_format($bundle->price, 2) }}
                                </p>
                                <a href="{{ route('bundles.show', $bundle->id) }}" 
                                   class="inline-block mt-4 px-6 py-2 border-2 border-indigo-600 text-white dark:text-white dark:border-teal-600 font-medium text-sm rounded-full hover:bg-white-600 hover:text-white dark:hover:bg-indigo-400 dark:hover:text-gray-900 transition-all duration-300">
                                    View Details
                                </a>
                            </div>
                        </div>
